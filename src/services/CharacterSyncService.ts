...
export class CharacterSyncService {
  ...
  private async syncAll(): Promise<void> {
    const rows = await this.db.all<Pick<CharacterSheetRow, 'id' | 'user_id'>>(
      'SELECT id, user_id FROM character_sheets',
    );
  ...
  async updateHitPoints(
    ...
    if (!Number.isFinite(current) || current < 0)
      throw new Error('Current HP must be a non-negative number');
