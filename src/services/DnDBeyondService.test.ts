...
describe('DnDBeyondService', () => {
  ...
  it('normalizes extended character fields from grouped D&D Beyond payloads', async () => {
    const payload = {
      ...
      data: {
        ...
        alignmentId: 6,
        inspiration: true,
        currencies: { cp: 1, sp: 2, gp: 30, pp: 4 },
        race: {
          fullName: 'Elf',
          weightSpeeds: { normal: { walk: 35 } },
          racialTraits: [{ definition: { name: 'Darkvision' } }],
        },
        stats: [
          { id: 1, value: 10 },
          { id: 2, value: 16 },
        ...
        classes: [
          {
            ...
            classFeatures: [{ definition: { name: 'Arcane Recovery' } }],
          },
        ],
        modifiers: {
          race: [
          ...
          item: [{ type: 'immunity', subType: 'poison' }],
        },
        inventory: [
          {
            id: 9,
        ...
        actions: {
          class: [
            { name: 'Arcane Recovery', limitedUse: { maxUses: 1, numberUsed: 0, resetType: 2 } },
          ],
        },
        classSpells: [
          { spells: [{ definition: { name: 'Fire Bolt', level: 0 }, prepared: true }] },
        ],
        background: { definition: { name: 'Sage', shortDescription: 'Researcher' } },
        traits: { personalityTraits: 'Curious\\nQuiet' },
      },
    };
    vi.mocked(global.fetch).mockResolvedValueOnce({
      ok: true,
    ...
    expect(character.resources?.[0]).toMatchObject({
    ...
    expect(character.features?.find((f) => f.name === 'Arcane Recovery')?.usesResourceName).toBe(
      'Arcane Recovery',
    );
  ...
  it('should throw error on failed fetch', async () => {
    ...
    await expect(service.fetchCharacter('12345')).rejects.toThrow(
      'D&D Beyond request failed (404)',
    );
  });
  ...
  it('should throw error on invalid response payload', async () => {
    ...
    await expect(service.fetchCharacter('12345')).rejects.toThrow(
      'Invalid D&D Beyond character response',
    );
  });
