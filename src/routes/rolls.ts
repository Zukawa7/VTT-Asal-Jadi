...
export function createRollsRouter(db: DatabaseService): Router {
  ...
  router.get('/:roomId', async (req, res, next) => {
    try {
      ...
      const session = await db.get<{ id: number }>(
        'SELECT id FROM game_sessions WHERE room_id = ?',
        [roomId],
      );
      if (!session) {
        res.json([]);
      ...
      const rows = await db.all<RollRow>(
        `SELECT id, character_id AS characterId, character_name AS characterName,
         ...
         FROM dice_rolls WHERE session_id = ? ORDER BY id DESC LIMIT 100`,
        [session.id],
      );
  ...
  router.get('/:roomId/analytics', async (req, res, next) => {
    try {
      ...
      if (!session) {
        res.json({ totalRolls: 0, averageResult: 0, criticalCount: 0, formulas: [] });
        return;
      }
      const summary = await db.get<{
        ...
        `SELECT COUNT(*) AS totalRolls, COALESCE(AVG(result), 0) AS averageResult,
                COALESCE(SUM(is_critical), 0) AS criticalCount FROM dice_rolls WHERE session_id = ?`,
        [session.id],
      );
