...
export function createCharacterRouter(
  ...
  router.get('/', auth, async (_req, res, next) => {
    try {
      const rows = await db.all<CharacterRow>(
        `SELECT id, character_data, user_id FROM character_sheets WHERE user_id = ?
        UNION ALL
        SELECT id, data AS character_data, user_id FROM characters
        WHERE user_id = ? AND NOT EXISTS (SELECT 1 FROM character_sheets WHERE character_sheets.id = characters.id)`,
  ...
  router.post('/import', auth, async (req, res, next) => {
    try {
      ...
      await db.run(
        `INSERT INTO character_sheets (id, user_id, character_data, last_synced) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(id) DO UPDATE SET user_id=excluded.user_id, character_data=excluded.character_data, last_synced=CURRENT_TIMESTAMP`,
        [String(character.id), userId, characterData],
      );
      await db.run(
        ...
        [String(character.id), userId, character.name, characterData],
      );
      res.json(character);
    } catch (error) {
      next(error);
  ...
  router.get('/:id/sheet', async (req, res, next) => {
    try {
      const row = await db.get<CharacterRow>(
        'SELECT id, character_data, user_id FROM character_sheets WHERE id = ?',
        [req.params.id],
      );
      if (!row) {
  ...
  router.put('/:id', auth, async (req, res, next) => {
    try {
      const userId = res.locals.user.id as number;
      const row = await db.get<CharacterRow>(
        'SELECT id, character_data, user_id FROM character_sheets WHERE id = ? AND user_id = ?',
      ...
      if (req.body?.hp && typeof req.body.hp.current === 'number')
        character.hp.current = Math.max(0, Math.min(character.hp.max, req.body.hp.current));
      if (req.body?.hp && typeof req.body.hp.temp === 'number')
        character.hp.temp = Math.max(0, req.body.hp.temp);
      ...
      if (Array.isArray(req.body?.equipment)) {
        if (req.body.equipment.length > 200) {
          res.status(400).json({ error: 'Equipment limit exceeded' });
          return;
        }
        character.equipment = req.body.equipment
          .map((item: Record<string, unknown>) => ({
            ...
            category:
              item.category === 'Equipment' || item.category === 'Backpack'
                ? item.category
                : undefined,
      ...
      if (req.body?.spellSlots && typeof req.body.spellSlots === 'object') {
        const slots = Object.fromEntries(
          Object.entries(req.body.spellSlots)
            .slice(0, 9)
            .map(([level, value]) => {
      ...
      if (Array.isArray(req.body?.conditions)) {
        if (req.body.conditions.length > 20) {
          res.status(400).json({ error: 'Condition limit exceeded' });
          return;
        }
        character.conditions = req.body.conditions
          .filter(
            (condition: unknown): condition is string =>
              typeof condition === 'string' && condition.length <= 50,
          )
          .slice(0, 20);
      }
      ...
      if (Array.isArray(req.body?.resources)) {
        if (req.body.resources.length > 50) {
          res.status(400).json({ error: 'Resource limit exceeded' });
          return;
        }
      ...
      if (req.body?.deathSaves && typeof req.body.deathSaves === 'object') {
        ...
        character.deathSaves = {
          successes: Math.max(
            0,
            Math.min(3, Number(ds.successes ?? character.deathSaves?.successes ?? 0)),
          ),
          failures: Math.max(
            0,
            Math.min(3, Number(ds.failures ?? character.deathSaves?.failures ?? 0)),
          ),
        };
      }
      ...
      await db.run(
        'UPDATE character_sheets SET character_data = ?, last_synced = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?',
        [JSON.stringify(character), req.params.id, userId],
      );
      onUpdate?.(character);
      res.json(character);
    } catch (error) {
  ...
  router.delete('/:id', auth, async (req, res, next) => {
    try {
      const result = await db.run('DELETE FROM character_sheets WHERE id = ? AND user_id = ?', [
        req.params.id,
        res.locals.user.id,
      ]);
      if (!result.changes) {
