...
function createDatabase() {
  ...
  return {
    ...
    all: async <T>(_sql: string, params: unknown[] = []) =>
      [...rows.values()].filter((row) => row.user_id === Number(params[0])) as T[],
...
function token(userId = 7) {
  return jwt.sign({ id: userId, username: `user${userId}` }, secret);
}
...
describe('typed character routes', () => {
  it('imports and lists only the authenticated user characters', async () => {
    ...
    const imported = await request(app)
      ...
      .send({ characterId: 'https://dndbeyond.com/characters/123' });
  ...
  it('preserves enrichment fields when updating equipment and handles new character fields', async () => {
    ...
    db.rows.set('123', {
      id: '123',
      user_id: 7,
      character_data: JSON.stringify(charWithEquipment),
    });
    ...
    expect(updated.status).toBe(200);
    expect(updated.body.equipment[0]).toMatchObject({
      id: '99',
      equipped: true,
      category: 'Equipment',
      isWeapon: true,
